...
export function JobFilters({ filters, onFiltersChange }: JobFiltersProps) {
  ...
  type JobType = (typeof jobTypeOptions)[number]['value'];
  ...
  type ExperienceLevel = (typeof experienceLevelOptions)[number]['value'];

  function updateFilters<K extends keyof JobSearchFilters>(
    key: K,
    value: JobSearchFilters[K] | undefined
  ) {
    onFiltersChange({ ...filters, [key]: value } as JobSearchFilters);
  }
  ...
  const handleJobTypeChange = (value: JobType | '') =>
    updateFilters('jobType', value ? value : undefined);

  const handleExperienceLevelChange = (value: ExperienceLevel | '') =>
    updateFilters('experienceLevel', value ? value : undefined);
      ...
      <div className='hidden md:flex items-center gap-1'>
        {filters.jobType && (
          <Badge variant='secondary' className='text-xs'>
            ...
            <button
              onClick={() => updateFilters('jobType', undefined)}
          ...
          <Badge variant='secondary' className='text-xs'>
            {
              experienceLevelOptions.find(
                (opt) => opt.value === filters.experienceLevel
              )?.label
            }
            <button
              onClick={() => updateFilters('experienceLevel', undefined)}
              className='ml-1 hover:text-destructive'
            >
              <X className='h-3 w-3' />
            </button>
